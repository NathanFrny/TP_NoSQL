    ...
    <table class="w3-table w3-striped">
        ...
        for ($i = 0; $i < $nb_res_on_page; $i++) {
            ...
            if ($import == 'confirm') {
                ...
                $document = [
                    'title' => $current_res['title'],
                    'year' => explode('-', $current_res['release_date'])[0],
                    'synopsis' => $current_res['overview'],
                    'id_tmdb' => $current_res['id'],
                    'production' => $dtls_film['production_companies'] ?? [],
                    'actors' => array_map(function($cast) { return $cast['name']; }, $cast_film['cast'] ?? [])
                ];
                ...
                if ($existingMovie) {
                    ...
                    if ($updateResult->getModifiedCount() == 0) {
                        echo "<p>Le film avec TMDB ID {$current_res['id']} n'a pas été mis à jour.</p>";
                    }
                } else {
                    $insertResult = $movies_collection->insertOne($document);
            
                    if ($insertResult->getInsertedCount() == 0) {
                        echo "<p>Le film avec TMDB ID {$current_res['id']} n'a pas été ajouté.</p>";
                    }
                }
            }
